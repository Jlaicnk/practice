...
const getUserRecommend = async () => {
  ...
  // 格式化 API 返回的数据
  const formattedRecommendations = response.recommendItems.map((item, index) => ({
    id: index + 1, // 自动生成一个唯一的 id
    title: item.name,
    description: item.desc,
    category: item.belong,
    views: item.count || 0, // 如果没有 count，设置为 0
  }));

  // 更新推荐数据
  recommendations.value = formattedRecommendations;
